...
function Confirmacao() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5001/api/ofertas/${ofertaId}`)
      .then((response) => {
        if (response.data) {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post("http://localhost:5001/api/negociacao", negociacaoData)
      .then((response) => {
        alert("Negociação criada com sucesso!");
        // Enviar a confirmação para o backend
        axios
          .post("http://localhost:5001/api/ofertas/confirmarOferta", {
            ofertaId,
          })
          .then((response) => {
        ...
        >
          ...
          <div className="mb-3">
            <div className="form-check">
              ...
              />
              <label htmlFor="aceito" className="form-check-label">
