...
const atualizarAvaliacoes = async (req, res) => {
  try {
    const { ratedId, ratingID } = req.params;

    const negociacao = await Negociacao.findOne({
      where: { negociacaoId: ratedId },
    });
    ...
    const isComprador = negociacao.usuarioIdComprador === Number(ratingID);
    const isVendedor = negociacao.usuarioIdVendedor === Number(ratingID);

    if (!isComprador && !isVendedor) {
      return res
        .status(403)
        .json({ error: "Usuário não autorizado a avaliar esta negociação" });
    }

    await Negociacao.update(
      {
        compradorAvaliou: isComprador ? true : negociacao.compradorAvaliou,
        vendedorAvaliou: isVendedor ? true : negociacao.vendedorAvaliou,
      },
      { where: { negociacaoId: ratedId } }
    );
